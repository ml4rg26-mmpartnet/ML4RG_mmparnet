...
def main() -> None:
    ...
    if args.task in {"multitask", "profile-only"}:
    ...
    if args.task in {"multitask", "binary-only"}:
        ...
        assert "binding_logit" not in out
        assert "binary_position_prob" not in out
        assert "alpha_bind" not in out
    if args.task == "multitask":
        assert out["binding_gate"].shape == (args.batch_size,)
    else:
        assert "binding_gate" not in out
    assert torch.isfinite(losses["loss"])
    print(f"\nPASS: {args.task} cross-attention forward/loss contract is valid.")
