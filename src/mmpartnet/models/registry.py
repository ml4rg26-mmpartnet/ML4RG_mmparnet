...
as default, and drop the loser once the comparison notebook captures it (see docs/MERGE_PLAN.md §8).
"""
...
REGISTRY: dict[str, HeadSpec] = {
    ...
    "early": HeadSpec(
        ...
        "concat[protein, pooled PARNET body] -> MLP (the mechanism-ladder floor / Christoph's baseline)."),
    "film": HeadSpec(
    ...
    "perres_bidir": HeadSpec(
        ...
        "bidirectional per-residue cross-attention (Diyi's design; ties per-residue in-distribution)."),
    "xattn": HeadSpec(
...
def build_head(name: str):
    ...
    Instantiate with the dims your run needs, e.g. ``build_head('conditioned')(dr=512, dp=640)``."""
    spec = head_spec(name)
